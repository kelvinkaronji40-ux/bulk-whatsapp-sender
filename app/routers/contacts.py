...
async def list_contacts(session: AsyncSession = Depends(get_session)):
    rows = (await session.execute(select(Contact).order_by(Contact.created_at.desc()))).scalars().all()
    return rows


@router.post("/", response_model=ContactOut, status_code=status.HTTP_201_CREATED)
async def create_contact(payload: ContactIn, session: AsyncSession = Depends(get_session)):
    ...
    c = Contact(phone=phone, name=payload.name, source=payload.source)
    session.add(c)
...
async def import_csv(file: UploadFile = File(...), session: AsyncSession = Depends(get_session)):
    ...
    result = await import_contacts_csv(session, content)
